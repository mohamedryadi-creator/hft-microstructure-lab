...
Six chapters on how a price is formed, what a market maker should quote, and what
an exchange should pay it — each one derived, implemented as tested code,
...
**63 tests**, all passing, none touching the network. **Six notebooks**, executed
...
Only `make data` touches the network.
...
src/hfx/
  ...
  mm/         the closed form, the discretised MDP, and the Q-learner
  design/     make-take fees as a Stackelberg problem
  pipeline/   the panel, the extraction, and the study
notebooks/    01..06, thin, executed, outputs committed
results/      panel.csv and curves.npz -- what makes the notebooks run offline
tests/        63 tests, none touching the network
```
